...
```sh
node serve.mjs 5180   # 동봉된 zero-dep 개발 서버 (no-store — 모듈 캐시 안 함)
# 또는 아무 정적 서버나: python3 -m http.server 5180
```

## 컨트롤

- **탭/클릭** — 원본 방식: 탭한 사분면의 코너에서 중앙으로 투척
- **드래그(플릭)** — 드래그 시작점에서 놓는 순간의 방향·속도로 투척 (마지막 ~130ms의 제스처 속도 인식)
- **휴대폰 흔들기** — 기기를 슉 움직이면 그 방향으로 투척 (devicemotion, iOS는 첫 터치에서 권한 요청, https 필요)
- 창 가장자리 = 물리 벽. 창 크기를 바꾸면 벽이 따라오고, 밖에 남은 주사위는 다시 던져 들어온다
...
- 보이는 화면 영역을 레이캐스트로 계산해 투명 벽 생성 — 창 가장자리가 곧 외벽,
  뷰포트 크기 변화에 자동 적응 (모바일 OK). 벽은 두께 20으로 고속 플릭에도 관통 불가,
  벽 접촉은 전용 ContactMaterial(고강성)로 박힘 방지, 2초 주기 패트롤이 이탈 주사위 회수
- 입력: 탭(코너 투척) / 드래그 플릭(방향·힘 인식) / devicemotion 흔들기(모바일)
